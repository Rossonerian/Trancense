...
export function ForgotPasswordForm() {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setLoading(true); setError(""); setMessage(""); try { const { error: authError } = await getSupabaseBrowserClient().auth.resetPasswordForEmail(email, { redirectTo: `${window.location.origin}/reset-password` }); if (authError) throw authError; setMessage(`If an account exists for ${email}, a password reset link has been sent.`); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to request a reset link."); } finally { setLoading(false); } }
