...
export function LoginForm({ demoMode = false, configurationMessage }: { demoMode?: boolean; configurationMessage?: string }) {
  ...
  const [error, setError] = useState(params.get("error") === "configuration" ? configurationMessage ?? "Supabase authentication is not configured for this deployment." : params.get("error") === "oauth" ? "Google sign-in could not be completed. Please try again." : configurationMessage ?? "");
