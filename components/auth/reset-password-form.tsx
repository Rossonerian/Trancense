...
export function ResetPasswordForm() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      void client.auth.getSession().then(({ data: { session } }) => {
        if (!mounted) return;
        setHasRecoverySession(Boolean(session) && hashType === "recovery");
        setChecking(false);
        if (!session || hashType !== "recovery") setError("This password reset link is invalid or expired. Request a new reset link.");
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setError(""); const passwordError = validatePasswordPair(password, confirm); if (passwordError) { setError(passwordError); return; } setLoading(true); try { const { error: authError } = await getSupabaseBrowserClient().auth.updateUser({ password }); if (authError) throw authError; setMessage("Password updated. You can now continue to your workspace."); setTimeout(() => { router.push("/overview"); router.refresh(); }, 500); } catch (caught) { setError(authErrorMessage(caught, "This reset link is invalid or expired.")); } finally { setLoading(false); } }
