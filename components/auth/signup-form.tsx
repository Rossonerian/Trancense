...
export function SignupForm({ demoMode = false }: { demoMode?: boolean }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const { data, error: authError } = await getSupabaseBrowserClient().auth.signUp({ email: fields.email, password: fields.password, options: { emailRedirectTo: `${origin}/auth/callback`, data: { full_name: fields.fullName, organization_name: fields.organization, job_role: fields.role, country: fields.country, phone: fields.phone } } });
  ...
  async function resend() { setResending(true); setError(""); const origin = getTrustedAppOrigin({ browserOrigin: window.location.origin }); const { error: resendError } = await getSupabaseBrowserClient().auth.resend({ type: "signup", email: fields.email, options: { emailRedirectTo: `${origin}/auth/callback` } }); setMessage(resendError ? resendError.message : `A new confirmation email was sent to ${fields.email}.`); setResending(false); }
