...
export function SignupForm({ demoMode = false, configurationMessage }: { demoMode?: boolean; configurationMessage?: string }) {
  ...
  async function resend() { setResending(true); setError(""); const origin = getTrustedAppOrigin({ browserOrigin: window.location.origin }); const { error: resendError } = await getSupabaseBrowserClient().auth.resend({ type: "signup", email: fields.email, options: { emailRedirectTo: `${origin}${googleOAuthCallbackPath}` } }); setMessage(resendError ? resendError.message : `A new confirmation email was sent to ${fields.email}.`); setResending(false); }
