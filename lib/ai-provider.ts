...
import { getWorkspaceSnapshot } from "./data-access";
...
function buildMessages(prompt: string, context: string) { return [{ role: "system", content: `You are Trancense's grounded energy-audit explainer. Never produce authoritative numerical values. Use only the supplied context, state uncertainty, cite IDs, and refuse statutory/legal/certification/control requests.\n\n${context}` }, { role: "user", content: safePrompt(prompt) }]; }

async function openRouter(prompt: string, context: string): Promise<{ text: string; model: string }> {
  ...
  for (const model of chain) {
    try {
      const response = await fetch("https://openrouter.ai/api/v1/chat/completions", { method: "POST", signal: timeoutSignal(), headers: { Authorization: `Bearer ${process.env.OPENROUTER_API_KEY}`, "Content-Type": "application/json", ...(process.env.OPENROUTER_SITE_URL ? { "HTTP-Referer": process.env.OPENROUTER_SITE_URL } : {}), ...(process.env.OPENROUTER_APP_NAME ? { "X-Title": process.env.OPENROUTER_APP_NAME } : {}) }, body: JSON.stringify({ model, messages: buildMessages(prompt, context), temperature: 0.1, max_tokens: MAX_OUTPUT_TOKENS }) });
...
async function compatibleChat(url: string, apiKey: string, model: string, prompt: string, context: string, headers: Record<string, string> = {}) {
  const response = await fetch(url, { method: "POST", signal: timeoutSignal(), headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json", ...headers }, body: JSON.stringify({ model, messages: buildMessages(prompt, context), temperature: 0.1, max_tokens: MAX_OUTPUT_TOKENS }) });
...
async function groq(prompt: string, context: string) { if (!process.env.GROQ_API_KEY) throw new Error("groq_key_missing"); return { text: await compatibleChat("https://api.groq.com/openai/v1/chat/completions", process.env.GROQ_API_KEY, process.env.GROQ_MODEL || "llama-3.1-8b-instant", prompt, context), model: process.env.GROQ_MODEL || "llama-3.1-8b-instant" }; }
async function gemini(prompt: string, context: string) { if (!process.env.GOOGLE_GENERATIVE_AI_API_KEY) throw new Error("gemini_key_missing"); const model = process.env.GEMINI_MODEL || "gemini-2.5-flash"; const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(process.env.GOOGLE_GENERATIVE_AI_API_KEY)}`, { method: "POST", signal: timeoutSignal(), headers: { "Content-Type": "application/json" }, body: JSON.stringify({ systemInstruction: { parts: [{ text: buildMessages(prompt, context)[0].content }] }, contents: [{ role: "user", parts: [{ text: safePrompt(prompt) }] }], generationConfig: { temperature: 0.1, maxOutputTokens: MAX_OUTPUT_TOKENS } }) }); if (!response.ok) throw new Error(`gemini_http_${response.status}`); const payload = await response.json() as { candidates?: Array<{ content?: { parts?: Array<{ text?: string }> } }> }; const text = payload.candidates?.[0]?.content?.parts?.map((part) => part.text ?? "").join("").trim(); if (!text) throw new Error("gemini_empty_response"); return { text, model }; }

export async function answerWithGrounding(prompt: string): Promise<AssistantResult> {
  const clean = safePrompt(prompt);
  const snapshot = await getWorkspaceSnapshot();
  const context = buildGroundedContext(snapshot);
  if (!clean || isUnsafeAssistantRequest(clean)) return { text: deterministicAssistantResponse(clean, snapshot), mode: "demo", provider: "demo", model: null, attempted: [] };
  ...
  for (const provider of providers) {
    ...
    try {
      const result = provider === "openrouter" ? await openRouter(clean, context) : provider === "groq" ? await groq(clean, context) : await gemini(clean, context);
  ...
  return { text: deterministicAssistantResponse(clean, snapshot), mode: "demo", provider: "demo", model: null, attempted };
}
